...
const ResidentModal = React.lazy(() => import("../ImportMasterList/Modal/AddResidentModal"));
const UploadModal = React.lazy(() => import("../ImportMasterList/Modal/UploadModal"));
const ResidentInfoModal = React.lazy(() => import("../ImportMasterList/Modal/ResidentInfoModal"));
const RFIDModal = React.lazy(() => import("../ImportMasterList/Modal/RFIDModal"));
const CardModal = React.lazy(() => import("../ImportMasterList/Modal/CardModal"));

const AddBenificiaryTable = ({
  LatestAssistances,
  isOpen,
  onClose,
  totalSelected,
  ...
  const payloads = {
    assistanceName: LatestAssistances?.name,
    _id: LatestAssistances?.assistanceId,
  };
  ...
  // ✅ FIXED: Remove the unnecessary fetch call, keep only localStorage update
  useEffect(() => {
  ...
  const selectAllOnPage = useCallback(() => {
    if (!ContinueResidents || ContinueResidents.length === 0) return;
    setSelectedIds((prev) => {
      if (benificiariyLimit != null) {
        const availableSlots = benificiariyLimit - prev.size;
        if (availableSlots <= 0) return prev;
        const idsToAdd = [];
        for (const resident of ContinueResidents) {
        ...
        const newSet = new Set(prev);
        ContinueResidents.forEach((resident) => {
          if (resident._id) newSet.add(resident._id);
        });
        return newSet;
      }
    });
  }, [ContinueResidents, benificiariyLimit]);
  ...
  const municipalityOptions = useMemo(
    () => ["All", "Naval", "Cabucgayan", "Caibiran", "Kawayan", "Almeria", "Culaba", "Maripipi", "Biliran"],
    []
  );
  ...
  const classificationOptions = useMemo(
    () => ["PWD", "Solo Parent", "Student", "Senior Citizen", "Indigent", "OFW", "Unemployed"],
    []
  );
      ...
      fetchTimeoutRef.current = setTimeout(async () => {
        ...
        const municipality =
          params.municipality && params.municipality !== "All"
            ...
            : "";
        const barangay =
          params.barangay && params.barangay !== "All"
            ? params.barangay
        ...
        try {
          await DisplayContinueSelectResidents(
            LatestAssistances.assistanceId,
            page,
            limit,
            search,
            municipality,
  ...
  const validateAndSetFile = useCallback(
    (file) => {
      ...
      if (file && validExtensions.includes(fileExtension)) {
        updateState({ file, uploadStatus: "idle" });
  ...
  const handleUpload = useCallback(async () => {
    ...
    if (!state.file) {
      showStatusMessage("failed", "No file selected", { title: "Upload Error" });
      return;
    }
    ...
    updateState({ uploadStatus: "uploading", modalLoading: { ...state.modalLoading, upload: true } });
  ...
  const validateResidentForm = useCallback(() => {
    ...
    if (errors.length > 0) {
      showStatusMessage("failed", errors.join(". "), { title: "Validation Error" });
      return false;
    }
    return true;
  ...
  const handleAddResident = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        const fullName = `${state.newResident.firstname} ${
          state.newResident.middlename ? state.newResident.middlename + " " : ""
        }${state.newResident.lastname}`;
        showStatusMessage("success", null, {
  ...
  const handleUpdateResident = useCallback(
    async (updatedResident) => {
      ...
      try {
        ...
        if (result.success) {
          const fullName = `${updatedResident.firstname} ${
            updatedResident.middlename ? updatedResident.middlename + " " : ""
          }${updatedResident.lastname}`;
          showStatusMessage("success", null, {
  ...
  const handleSearchChange = useCallback(
    (value) => {
      ...
      searchDebounceTimeoutRef.current = setTimeout(() => {
        ...
        handleFetchData({ search: value.trim() !== "" ? value : undefined });
      }, 500);
    },
  ...
  const handleMunicipalityChange = useCallback(
    (value) => {
      ...
      updateState({ selectedMunicipality: value, selectedBarangay: "All" });
      if (value === "All") {
      ...
      paginationRef.current.shouldResetToPage1 = true;
      handleFetchData({ municipality: value !== "All" ? value : undefined, barangay: undefined });
    },
  ...
  const handleBarangayChange = useCallback(
    (value) => {
      ...
      paginationRef.current.shouldResetToPage1 = true;
      handleFetchData({ barangay: value !== "All" ? value : undefined });
    },
  ...
  const handleViewResidentInfo = useCallback(
    (resident) => {
      if (!resident) {
        ...
        showStatusMessage("failed", "No resident information available.", { title: "View Error" });
        return;
      }
  ...
  const handleAssignRFID = useCallback(
    (resident) => {
      if (!resident) {
        console.error("No resident provided");
        showStatusMessage("failed", "No resident selected for RFID assignment.", { title: "RFID Error" });
        return;
      }
      updateState({
  ...
  const saveRFIDAssignment = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        const fullName = `${state.selectedResident.firstname} ${
          state.selectedResident.middlename ? state.selectedResident.middlename + " " : ""
        }${state.selectedResident.lastname}`;
        showStatusMessage("success", null, {
  ...
  const handleGenerateCard = useCallback(
    (resident) => {
      if (!resident) {
        showStatusMessage("failed", "No resident selected for card generation.", { title: "Card Generation Error" });
        return;
      }
  ...
  const handlePrintCard = useCallback(() => {
    ...
    try {
      ...
      printWindow.document.write(`
        <html>
          <head><title>Print Resident Card</title></head>
          <body>
      ...
      showStatusMessage("failed", error.message || "Failed to print card.", { title: "Print Error" });
    }
  }, [showStatusMessage]);
  ...
  const handleDownloadCard = useCallback(() => {
    ...
    try {
      ...
      showStatusMessage("failed", error.message || "Failed to download card.", { title: "Download Error" });
    }
  }, [showStatusMessage]);
  ...
  // Kung hindi bukas ang modal, huwag mag-render
  if (!isOpen) return null;

  // Loading states (panloob ng modal)
  if (!state.uiReady) {
    return (
    ...
    return (
      <div className="fixed inset-0 z-[999] overflow-y-auto bg-black/50 backdrop-blur-sm">
        <div className="flex min-h-screen items-center justify-center p-4">
          <div className="w-full max-w-7xl rounded-lg bg-white p-4 dark:bg-gray-800">
            <div className="mb-8">
  ...
  // Main render na may modal wrapper
  return (
    <div className="fixed inset-0 z-[999] overflow-y-auto bg-black/50 backdrop-blur-sm">
      <div className="flex min-h-screen items-center justify-center p-4">
        <div className="relative w-full max-w-7xl rounded-lg bg-white shadow-xl dark:bg-gray-800">
          {/* Close button - now clears localStorage */}
          <button
            onClick={() => {
              localStorage.removeItem("selectedResidentIds");
              onClose();
            }}
          ...
          {/* Original content (with adjusted padding) */}
          <div className="p-4 md:p-8">
            <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-center">
              <div>
                ...
                <p className="mt-1 text-gray-500 dark:text-gray-400">
                  Manage the municipality's resident list.
                </p>
              </div>
              {/* Done button removed */}
            </div>
            ...
            >
              <MainTable
                SelectedSize={totalSelected} // ← direktang gamit ang size
                beneficiaryLimit={beneficiaryLimit}
                data={ContinueResidents}
                LatestAssistances={payloads}
          ...
          <Suspense fallback={null}>
            ...
            {state.showUploadModal && (
              <UploadModal
                ...
                resetFile={() => updateState({ file: null, uploadStatus: "idle" })}
...
export default AddBenificiaryTable;